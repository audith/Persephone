	...
	public function setupSources ( $options )
	{
		...
		if ( !$cOptions[ 'groupsOnly' ] && isset( $_GET[ 'f' ] ) )
		{
			...
			if ( // verify at least one file, files are single comma separated,
				...
				!preg_match( '/^[^,]+\\.(css|js)(?:,[^,]+\\.\\1)*$/', $_GET[ 'f' ], $m ) // no "//"
			...
			{
				// check for validity
				if ( is_string( $_GET[ 'b' ] ) && preg_match( '@^[^/]+(?:/[^/]+)*$@', $_GET[ 'b' ] ) && false === strpos( $_GET[ 'b' ], '..' ) && $_GET[ 'b' ] !== '.' )
				{
					// valid base
					$base = "/{$_GET[ 'b' ]}/";
				}
			...
			if ( null !== $firstMissingResource )
			{
				array_unshift(
					...
					new Minify_Source( array(
					                        'id'           => 'missingFile' // should not cause cache invalidation
					                        ,
					                        'lastModified' => 0 // due to caching, filename is unreliable.
					                        ,
