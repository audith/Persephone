...
graphicsDir : '/public/images/highslide/',
...
css: function(el, prop) {
	if (el.style[prop]) {
		...
		if (prop == 'filter'){val = val.replace(/alpha\(opacity=([0-9]+)\)/,
...
undim : function(key) {
	...
	if (hs.geckoMac && hs.dimmingGeckoFix){hs.dimmer.style.display = 'none';} else {hs.animate(hs.dimmer, { opacity: 0 }, hs.dimmingDuration, null, function() {
...
calcExpanded: function() {
	...
	// get alignment
	if (exp.align == 'center'){this.justify = 'center';}else if (new RegExp(this.lt).test(exp.anchor)){this.justify = null;}else if (new RegExp(this.rb).test(exp.anchor)){this.justify = 'max';}
...
afterExpand : function() {
	...
	if (this.iframe) {
		...
		if (hs.ie && (typeof this.isClosing != 'boolean')){this.iframe.style.width = (this.objectWidth - 1) +'px'; // hasLayout
}
	}
...
htmlPrepareClose : function() {
	...
	if (this.objectType == 'swf') {
		try { hs.$(this.body.id).StopPlay(); } catch (e) {}
	}
