	...
	private function linkinfo__do_recache ( $params = array() )
	{
		$this->Registry->Db->cur_query = array(
				...
				'fields' => array(
						...
						'm_ip_address' => new Zend_Db_Expr( "INET_NTOA( CONV( " . $this->Registry->Db->adapter->quoteIdentifier( "m_ip_address" ) . ", 2, 10 ) )" ),
					),
				'where'  => array(
		...
		foreach ( $result as $r )
		{
			...
			$this->Registry->Db->cur_query = array(
					...
					'fields'    => array(
							"type_extension",
							"type_hex_id"
								=> new Zend_Db_Expr("HEX(" . $this->Registry->Db->adapter->quoteIdentifier("type_hex_id") . ")" ),
							"type_hex_id_offset",
