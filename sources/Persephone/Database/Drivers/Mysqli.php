...
class Mysqli extends \Persephone\Database
{
	...
	public function __construct ( \Persephone\Registry $Registry )
	{
		...
		$this->adapter = Zend_Db::factory( "Mysqli", $params );
		...
		if ( IN_DEV )
		{
			...
			$this->adapter->setProfiler( $_profiler );

			# Check connection
			$_connection = $this->adapter->getConnection();
	...
	protected final function simple_replace_query ( $sql )
	{
		...
		try
		{
			$stmt = $this->adapter->query( $this->cur_query, $sql['set'] );
			...
			if ( is_array( $_table ) and count( $_table ) )
			{
				foreach ( $_table as $_alias=>$_table_name )
				{
					...
					else
					{
						$_tables[] = $this->adapter->quoteIdentifier( $this->attach_prefix( $_table_name ), true )
							. " AS "
							. $this->adapter->quoteIdentifier( $_alias, true );
					}
				}
			}
	...
	protected function simple_alter_table ( $sql )
	{
		...
		try
		{
			$stmt = new Zend_Db_Statement_Mysqli( $this->adapter, $this->cur_query );
			return $stmt->execute();
		}
		catch ( Zend_Db_Exception $e )
		{
		...
		foreach ( $struct['tables'] as $table=>$data )
		{
			...
			foreach ( $data['col_info'] as $_column_name=>$_column_struct )
			{
				...
				switch ( strtolower( $_column_struct['type'] ) )
				{
					...
					case 'decimal':
						$this->cur_query .=
							$this->adapter->quoteIdentifier( $_column_name, true ) . " "
							. $_column_struct['type']
