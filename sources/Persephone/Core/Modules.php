...
class Modules
{
	...
	public function __construct ( \Persephone\Registry $Registry )
	{
		...
		if ( !defined( "SITE_URL" ) )
		{
			$_connection_type = $this->cur_module[ 'm_enforce_ssl' ]
				? "https"
				: "http";
	...
	public function content__build_http_headers ( &$m )
	{
		...
		$headers = array(
			...
			"pragma"           => $m[ 'm_enable_caching' ]
				? ""
				: "no-cache",
			"expires"          => $m[ 'm_enable_caching' ]
				? gmdate( "r", time() + 2592000 )
				: gmdate( "r", time() - 86400 ),
	...
	{
		$_cache_key_name__subroutines__content_for = "subroutines__content_for__" . md5(
			$m[ 'm_unique_id' ] . $m[ 'running_subroutine' ][ 's_name' ] . $this->Registry->config[ 'page' ][ 'request' ][ 'path' ] . $m[ 'running_subroutine' ][ 'page_nr_requested' ]
		);

		$_cache_key_name__subroutines__keys_for = "subroutines__keys_for__" . md5(
			$m[ 'm_unique_id' ] . $m[ 'running_subroutine' ][ 's_name' ] . $this->Registry->config[ 'page' ][ 'request' ][ 'path' ]
		);

		if ( !$m[ 'running_subroutine' ][ 'content' ][ 'parsed' ] = $this->Registry->Cache->cache__do_get( $_cache_key_name__subroutines__content_for, true )
		     or
		...
		)
		{
			...
			{
				$_fields_to_fetch = array_merge(
					array( "id", "tags", "timestamp", "submitted_by", "status_published", "status_locked" ),
					$s_data_definition_for_join = array_keys( $m[ 'running_subroutine' ][ 's_data_definition' ] )
				);
			...
			if ( isset( $m[ 'running_subroutine' ][ 's_fetch_criteria_parsed' ][ 'policies' ] )
			     and
			...
			)
			{
				...
				foreach ( $m[ 'running_subroutine' ][ 's_fetch_criteria_parsed' ][ 'policies' ] as $_policy )
				{
					...
					if ( !empty( $_list_of_tables_to_process ) )
					{
						foreach ( $_list_of_tables_to_process as $_table_alias )
						{
							...
							$_connector_joins[ ]                       = array(
								...
								$this->Registry->Db->adapter->quoteIdentifier( "master.id" ) . " = " . $this->Registry->Db->adapter->quoteIdentifier( $_table_alias . ".ref_id" ),
					...
					$_subquery                                 = $this->Registry->Db->adapter->select()->from(
			...
			# + UNION
			$_base_query = $this->Registry->Db->adapter->select()->union( $_subqueries );
			...
			if ( !$_cache_of_keys_for_the_result_set = $this->Registry->Cache->cache__do_get(
				$_cache_key_name__subroutines__keys_for,
				true
			)
			)
			{
				$this->Registry->Db->cur_query = $this->Registry->Db->adapter->select()->from(
						new Zend_Db_Expr( "( " . strval( $_base_query ) . " )" ),
						array(
						     'list' => new Zend_Db_Expr( "DISTINCT( " . $this->Registry->Db->adapter->quoteIdentifier( "id" ) . " )" ),
						     // 'count' => new Zend_Db_Expr( "COUNT( DISTINCT( " . $this->Registry->Db->adapter->quoteIdentifier( "id" ) . " ) )" ),
						)
					);
				try
				{
					$_r = $this->Registry->Db->adapter->query( $this->Registry->Db->cur_query )->fetchAll();
			...
			{
				$this->Registry->Db->cur_query->where( $this->Registry->Db->adapter->quoteIdentifier( "id" ) . " IN ( " . $_keys_to_fetch . " )" );
			}
			else
			{
				...
				if ( !$m[ 'running_subroutine' ][ 'content' ][ 'count' ] and $m[ 'running_subroutine' ][ 'page_nr_requested' ] > 1 )
				{
					$this->Registry->http_redirect(
						$this->Registry->config[ 'page' ][ 'request' ][ 'scheme' ] . '://' . $this->Registry->config[ 'page' ][ 'request' ][ 'host' ] .
			...
			try
			{
				$result = $this->Registry->Db->adapter->query( $this->Registry->Db->cur_query )->fetchAll();
				$this->Registry->Db->query_count++;
			}
			catch ( Zend_Db_Exception $e )
			{
					...
					if ( array_key_exists( $_field_name, $m[ 'running_subroutine' ][ 's_data_definition' ] ) )
					{
						...
						if ( ( $_strpos_dot_in_field_name = strpos( $_field_name, "." ) ) !== false )
						{
							...
							if ( !is_null( $_field_content ) )
							{
								$_node_reference
								[ $_new_field_name ]
								[ $_raw_content[ $_field_name_exploded[ 0 ] . '.id' ] ] = $_processor_instance->get__do_process(
									array(
	 ...
	 *
	 * @param   array       $m                                  Module info
	 * @param   integer     $ref_id                             Row 'id'
	 * @param   array       $fields_to_fetch                    DFD, to fetch from
	 * @param   boolean     $_callback_via_data_processor       Are we called via data-processor?
	 *
	...
	public function content__do_process__by_ref_id ( &$m, $ref_id, $fields_to_fetch = array(), $_callback_via_data_processor = true )
	{
		...
		if ( !empty( $fields_to_fetch ) )
		{
			foreach ( $fields_to_fetch as $_table_alias )
			{
				...
				$_connector_joins[ ] = array(
					array( $_table_alias => $this->Registry->Db->attach_prefix( $_attach_prefix_reference_makes_us_do_this ) ),
					$this->Registry->Db->platform->quoteIdentifier( "master.id" ) . " = " . $this->Registry->Db->platform->quoteIdentifier( $_table_alias . ".ref_id" ),
					array(), // No list of columns to fetch, we place that in WHERE clause
				);
			}
		}
		...
		$_attach_prefix_reference_makes_us_do_this = "mod_" . $m[ 'm_unique_id_clean' ] . "_master_repo";
		$this->Registry->Db->cur_query = $this->Registry->Db->sql->select()->from(
			array( 'master' => $this->Registry->Db->attach_prefix( $_attach_prefix_reference_makes_us_do_this ) ),
			$_fields_to_fetch
		);
		...
		# + Where
		$this->Registry->Db->cur_query->where( $this->Registry->Db->platform->quoteIdentifier( "master.id" ) . "=" . $this->Registry->Db->adapter->quote( $ref_id, "INTEGER" ) );
		...
		foreach ( $m[ 'running_subroutine' ][ 'content' ][ 'raw' ] as $_raw_content )
		{
			...
			foreach ( $_raw_content as $_field_name => $_field_content )
			{
				...
				{
					$_field_name_exploded = explode( ".", $_field_name );
					if ( array_key_exists( $_field_name_exploded[ 0 ], $m[ 'm_data_definition' ] )
					     and
	...
	public function modules__do_load ( &$m )
	{
		...
		if ( $m[ 'm_type' ] == 'built-in' )
		{
			...
			$_module_handler_class = '\Persephone\Modules\\' . ucfirst( $m[ 'm_name' ] ) . '\Handler';
			$m[ 'han' ]            = new $_module_handler_class( $this->Registry );
		...
		# Module URL prefix
		$_connection_type    = $m[ 'm_enforce_ssl' ]
			? "https"
			: "http";
	...
	{
		if ( isset( $this->Registry->Cache->cache[ 'modules' ][ 'by_name' ] ) and
		     is_array( $this->Registry->Cache->cache[ 'modules' ][ 'by_name' ] ) and !empty( $this->Registry->Cache->cache[ 'modules' ][ 'by_name' ] )
		)
		{
			$active_modules = array();
			foreach ( $this->Registry->Cache->cache[ 'modules' ][ 'by_name' ] as $m_name => $m_data )
			{
				if ( IN_DEV or $m_data[ 'm_is_enabled' ] == 1 )
				{
					$active_modules[ $m_name ] = $m_data;
				}
			}
	 ...
	 *
	 * @param    array     $m   Module information
	 *
	...
	private function subroutines__do_process ( &$m )
	{
		...
		{
			if ( preg_match(
				'#^' . $_s_data[ 's_pathinfo_uri_schema_parsed' ] . '$#i',
				str_replace( "/" . $m[ 'm_name' ] . "/", "", $this->Registry->config[ 'page' ][ 'request' ][ 'path' ] ),
				$_match
			)
			)
			{
				...
				# Page number
				$m[ 'running_subroutine' ][ 'page_nr_requested' ] = isset( $_GET[ '_page' ][ $m[ 'running_subroutine' ][ 's_name' ] ] )
	...
	private function subroutines__fetch_criteria__do_parse ( &$m )
	{
		...
		if ( isset( $m[ 'running_subroutine' ][ 's_fetch_criteria' ][ 'rules' ] ) and count( $m[ 'running_subroutine' ][ 's_fetch_criteria' ][ 'rules' ] ) )
		{
			foreach ( $m[ 'running_subroutine' ][ 's_fetch_criteria' ][ 'rules' ] as $_rule )
			{
				...
				$_rule__parsed = $this->Registry->Db->platform->quoteIdentifier( $_rule[ 'field_name' ] ) . " " . html_entity_decode( $_rule[ 'math_operator' ], ENT_COMPAT, "UTF-8" );
				...
				if ( $_rule[ 'math_operator' ] != 'IS NULL' and $_rule[ 'math_operator' ] != 'IS NOT NULL' )
				{
					...
					if ( $_rule[ 'type_of_expr_in_value' ] == 'math' )
					{
						...
						)
						{
							...
							$_rule__parsed .= $this->Registry->Db->platform->quoteValue( eval( "return " . $_rule[ 'value' ] . ";" ), "FLOAT" );
						}
						else
						{
							# INTEGER
							$_rule[ 'value' ] = $this->Registry->Input->clean__makesafe_mathematical( $_rule[ 'value' ] ); // Cleanup
							$_rule__parsed .= $this->Registry->Db->platform->quoteValue( eval( "return " . $_rule[ 'value' ] . ";" ), "INTEGER" );
						}
					}
					elseif ( $_rule[ 'type_of_expr_in_value' ] == 'zend_db_expr' )
					{
						# Zend_Db_Expr
						$_zend_db_sql_expression = new \Zend\Db\Sql\Expression( $_rule[ 'value' ] );
						$_rule__parsed .= $_zend_db_sql_expression->getExpression();  // @todo FIX
					}
					else
					{
						# GENERIC
						$_rule__parsed .= $this->Registry->Db->platform->quoteValue( $_rule[ 'value' ] );
					}
				}
			...
			foreach ( $m[ 'running_subroutine' ][ 's_fetch_criteria' ][ 'policies' ] as $_policy )
			{
				...
				$_parsed_policy = preg_replace_callback(
					...
					function( $matches ) use ( $m ) {
						return $m[ 'running_subroutine' ][ 's_fetch_criteria_parsed' ][ 'rules' ][ intval( $matches[0] )-1 ]['parsed_rule'];
					} ,
					$_policy
				);
	 ...
	 *
	 * @param    array    $m                Module info
	 * @param    array    $node_reference   Where to attach the parsed data
	 * @param    array    $raw_content      Raw data to parse
	 *
	 * @return   void
	 */
		...
		foreach ( $raw_content as $_field_name => $_field_content )
		{
			switch ( $_field_name )
			{
				...
				case 'status_published':
					$node_reference[ 'status_published' ] = $_field_content
						? 1
						: 0;
					break;

				case 'status_locked':
					$node_reference[ 'status_locked' ] = $_field_content
						? 1
						: 0;
					break;
			}
