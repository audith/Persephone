...
class Modules
{
	...
	**/
	...
	/**
	 * WRAPPER: Fetches content for module subroutines, using appropriate set of rules/policies
	 *
	...
	public function content__do ( &$m )
	{
		...
		if ( isset( $m['running_subroutine'] ) and $m['running_subroutine']['s_service_mode'] == 'write-only' and $action == 'default' )
		{
			return null;
		}
		...
		if ( isset( $m['running_subroutine'] ) )
		{
			...
			else
			{
				if ( $action == 'default' )
				{
					$return = $this->subroutines__content__do_get( $m );
				}
				elseif ( $action == 'put' )
				{
	 ...
	 */
	public function content__do__by_ref_id ( &$m , $ref_id , $fields_to_fetch = array() , $_callback_via_data_processor = TRUE )
	{
	...
	/**
	 * Fetches content for module subroutines, using appropriate set of rules/policies
	 *
	 * @param   array    REF: Module info
	 * @return  mixed    (mixed) Fetched content on success; (boolean) FALSE otherwise
	 *
	 * @todo   404 not found situation is not properly handled
	 */
	private function subroutines__content__do_get ( &$m )
	{
		...
		)
		{
			...
			if ( $m['running_subroutine']['s_service_mode'] == 'read-write' )
			{
				...
				$return['content'] = $this->content__do__by_ref_id( $m , $m['running_subroutine']['request']['id'] , $_fields_to_fetch , FALSE );
			...
			if ( count( $m['running_subroutine']['s_fetch_criteria_parsed']['limit'] ) == 2 )
			{
				...
				$_keys_to_fetch = implode(
						...
						array_map(
								create_function(
										'$value' ,
										'return $GLOBALS["API"]->Db->db->quote( $value , "INTEGER" );'
									),
								array_slice(
										$_cache_of_keys_for_the_result_set,
	...
	/**
	 * Parses a module subroutine fetch criteria to a valid WHERE sql clause
	 *
	 * @param    array    REF: Module info
	 * @return   void
	 */
	private function subroutines__fetch_criteria__do_parse ( &$m )
	{
		...
		if ( isset( $m['running_subroutine']['s_fetch_criteria']['rules'] ) and count( $m['running_subroutine']['s_fetch_criteria']['rules'] ) )
		{
			foreach ( $m['running_subroutine']['s_fetch_criteria']['rules'] as $_rule )
			{
				...
				$this->_tmp =& $m['running_subroutine']['request'];    // A way around... Otherwise Lambda-function will turn it to a nightmare
				$_rule['value'] = preg_replace_callback(
						'#<backreference>((?>[a-z][a-z0-9_]+))<\/backreference>#' ,
						// PHP 5.3.0 or later:
						//function( $matches ) {
						//		return $GLOBALS['API']->Modules->_tmp[ $matches[1] ];
						//	} ,
						create_function(
								'$matches' ,
								'return $GLOBALS["API"]->Modules->_tmp[ $matches[1] ];'
		...
		if ( isset( $m['running_subroutine']['s_fetch_criteria']['policies'] ) and count( $m['running_subroutine']['s_fetch_criteria']['policies'] ) )
		{
			...
			{
				$this->_tmp =& $m['running_subroutine']['s_fetch_criteria_parsed']['rules'];                 // A way around... Otherwise Lambda-function will turn it to a nightmare
				...
				$_parsed_policy = preg_replace_callback(
						...
						// PHP 5.3.0 or later:
						//function( $matches ) {
						//		return $GLOBALS['API']->Modules->_tmp[ intval( $matches[0] )-1 ]['parsed_rule'];
						//	} ,
