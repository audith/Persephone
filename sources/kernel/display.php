...
class Display
{
	...
	public function do_display ()
	{
		...
		if ( $this->Registry->Input->headers['request']['_is_ajax'] or $this->output_mode == 'json' )
		{
			...
			print Zend_Json::encode( $this->content['content'] );
		}
		else
		{
		...
		if ( is_array( $this->cur_skin['set_assets'] ) and ! empty( $this->cur_skin['set_assets'] ) )
		{
			$this->skin_assets = array(
					'css'  =>
						...
						. SITE_URL . "/public/min/index.php?f=",
...
if ( $this->Registry->Modules->cur_module['m_name'] == 'acp' )
{
	...
	private function misc__pager__do_parse ( $total_nr_of_items , $nr_of_items_per_page , $range , $current_page )
	{
		...
		$components[] = array(
				'value'        => $current_page,
				'_is_first'    => ( $_i == 1 ? true : false ),
				'_is_last'     => ( $_i == $total_nr_of_pages ? true : false ),
				'_is_current'  => $_current_page_is_off_range ? false : true,
				'_is_dump'     => false,
			);
	...
	public final function smarty__init ( $skin = null )
	{
		...
		{
			throw new Exception( "Display: Couldn't instantiate Smarty template-engine!" );
			return false;
		}
	...
	public function smarty__plugin__function__pager ( $params , $smarty_object )
	{
		...
		{
			throw new Exception ("Display Engine: FAILED to parse Pager information!");
			return false;
		}
		...
		if ( isset( $this->pager_instances_container[ $params['instance'] ] ) )
		{
			...
			if ( $_instance['params'] !== $params )
			{
				throw new Exception ( "Pager-Instance on " . $params['instance'] . " is in use! Choose another instance identifier!" );
				exit;
			}
		}
	...
	public function smarty__resource__bits__check_tpl_name ( $tpl_name , $smarty_object )
	{
		...
		if ( empty( $tmp[1] ) )
		{
			throw new Exception( "Display: Templates - Empty 'bit_name' detected!" );
			return false;
		}
		if ( !empty( $tmp[0] ) and !preg_match( '/^[a-z0-9]{32}$/i', $tmp[0] ) )
		{
			throw new Exception( "Display: Templates - Empty 'm_unique_id_clean' detected!" );
		}
		...
		$template_bit_information['t_type'] =  ( ! isset( $tmp[2] ) or empty( $tmp[2] ) ) ? "tpl" : $tmp[2];
		unset( $tmp );
		...
		if ( file_exists( $_fs__full_path['primary'] ) and is_file( $_fs__full_path['primary'] ) and ( filemtime( $_fs__full_path['primary'] ) + $smarty_object->cache_lifetime > UNIX_TIME_NOW ) )
		{
		...
		if ( $template_bit_information['fs__exists'] === false and file_exists( $_fs__full_path['secondary'] ) and is_file( $_fs__full_path['secondary'] ) and ( filemtime( $_fs__full_path['secondary'] ) + $smarty_object->cache_lifetime > UNIX_TIME_NOW ) )
		{
			$template_bit_information['fs__exists']    = true;
		...
		if ( !$template_bit_information['fs__exists'] )
		{
			...
			if ( !file_exists( $_tpl_dir = dirname( $template_bit_information['fs__full_path'] ) ) or !is_dir( $_tpl_dir ) )
			{
				...
				{
					throw new Exception( "Display: Couldn't create Smarty::template_dir folder!" );
				}
			}
